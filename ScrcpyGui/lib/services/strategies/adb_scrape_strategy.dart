...
class AdbScrapeStrategy implements IconFetchStrategy {
  ...
  Future<void> fetchAll({
    ...
    debugPrint('[AdbScrapeStrategy] Starting fetch: $total packages, batchSize=$batchSize, pending=${pendingIcon.length}');
    final stopwatch = Stopwatch()..start();

    for (var i = 0; i < packages.length; i += batchSize) {
      if (isCancelled()) {
        debugPrint('[AdbScrapeStrategy] Cancelled at chunk ${i ~/ batchSize}');
        return;
      }

      final chunkIndex = i ~/ batchSize;
      final chunk = packages.sublist(i, (i + batchSize).clamp(0, packages.length));
      debugPrint('[AdbScrapeStrategy] Chunk $chunkIndex: firing ${chunk.length} futures (${chunk.where(pendingIcon.contains).length} ADB, ${chunk.where((p) => !pendingIcon.contains(p)).length} cached)');
      ...
      final futures = chunk.map((pkg) async {
        ...
        if (!pendingIcon.contains(pkg)) {
          ...
          debugPrint('[AdbScrapeStrategy] $pkg => ${result != null ? "icon" : "null"} (${stopwatch.elapsedMilliseconds}ms)');
        }
        return MapEntry(pkg, result);
      });

      final results = await Future.wait(futures);
      final chunkMs = stopwatch.elapsedMilliseconds - chunkStart;
      debugPrint('[AdbScrapeStrategy] Chunk $chunkIndex done in ${chunkMs}ms');
